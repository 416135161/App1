...
public class AlterSetting extends Activity implements OnCheckedChangeListener, RadioGroup.OnCheckedChangeListener,
	OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		check_send_msg.setChecked(UserSession.getCheckSendMsg(getApplicationContext()));
	...
	protected Dialog onCreateDialog(int id) {
		...
		switch (id) {
			...
			case TIME_DIALOG1:
				...
				dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						text1.setText(transformStyle(hourOfDay) + ":" + transformStyle(minute));
						UserSession.setTime(getApplicationContext(), (String)text1.getText(), 1);
					}
			...
			case TIME_DIALOG2:
				...
				dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						text2.setText(transformStyle(hourOfDay) + ":" + transformStyle(minute));
						UserSession.setTime(getApplicationContext(), (String)text2.getText(), 2);
					}
				}, c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), true);
				break;
			case TIME_DIALOG3:
				...
				dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						text3.setText(transformStyle(hourOfDay) + ":" + transformStyle(minute));
						UserSession.setTime(getApplicationContext(), (String)text3.getText(), 3);
					}
				}, c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), true);
				break;
			case TIME_DIALOG4:
				...
				dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						text4.setText(transformStyle(hourOfDay) + ":" + transformStyle(minute));
						UserSession.setTime(getApplicationContext(), (String)text4.getText(), 4);
					}
				}, c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), true);
				break;
			case TIME_DIALOG5:
				...
				dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
					public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
						text5.setText(transformStyle(hourOfDay) + ":" + transformStyle(minute));
						UserSession.setTime(getApplicationContext(), (String)text5.getText(), 5);
					}
				}, c.get(Calendar.HOUR_OF_DAY), c.get(Calendar.MINUTE), true);
				break;
	...
	private void showSendReportPhoneDlg() {
		if (dialog == null) {
			View view = getLayoutInflater().inflate(R.layout.send_report_phone, null);
			...
			button.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (TextUtils.isEmpty(phoneNo1) || phoneNo1.length() != 11) {
						NormalUtil.displayMessage(getApplicationContext(), "输入的非手机号码！");
						((Dialog)dialog).show();
						return;
					}
					UserSession.setSendReportPhoneNo(getApplicationContext(), phoneNo1);
				}
			});
			text1.setText(UserSession.getPhoneNo(getApplicationContext()));
			dialog = new AlertDialog.Builder(AlterSetting.this).setView(view).create();
